...
event bro_init()
{
	...
	#######################################################################################
	#query["osquery::chrome_extensions"] =  "SELECT name,author,path FROM chrome_extensions";

	########################################################################################
	#query["osquery::cpuid"] =  "SELECT feature,value FROM cpuid";

	#######################################################################################
	#query["osquery::crontab"] =  "SELECT event,hour,command FROM crontab";

	#######################################################################################
	#query["osquery::disk_encryption"] =  "SELECT name,uuid,encrypted FROM disk_encryption";

	########################################################################################
	#query["osquery::etc_hosts"] =  "SELECT address,hostnames FROM etc_hosts";

	########################################################################################
	#query["osquery::etc_protocols"] =  "SELECT name,number FROM etc_protocols";

	#######################################################################################
	#query["osquery::etc_services"] =  "SELECT name,port,protocol FROM etc_services";

	#######################################################################################
	#query["osquery::file_events"] =  "SELECT target_path,action,time FROM file_events";

	#######################################################################################
	#query["osquery::firefox_addons"] =  "SELECT name,source_url,location FROM firefox_addons";

	#######################################################################################
	#query["osquery::groups"] =  "SELECT gid,groupname FROM groups";

	#######################################################################################
	#query["osquery::hardware_events"] =  "SELECT action,model,vendor FROM hardware_events";

	#######################################################################################
	#query["osquery::interface_address"] =  "SELECT interface,address FROM interface_address";

	#######################################################################################
	#query["osquery::interface_details"] =  "SELECT interface,mac,mtu FROM interface_details";

	#######################################################################################
	#query["osquery::kernel_info"] =  "SELECT version,path,device FROM kernel_info";

	#######################################################################################
	#query["osquery::last"] =  "SELECT username,pid,host FROM last";

	#######################################################################################
	#query["osquery::listening_ports"] =  "SELECT pid,port,protocol FROM listening_ports";

	#######################################################################################
	#query["osquery::logged_in_users"] =  "SELECT user,host,time FROM logged_in_users";

	########################################################################################
	#query["osquery::mounts"] =  "SELECT device,path FROM mounts";

	########################################################################################
	#query["osquery::opera_extensions"] =  "SELECT name,description,author FROM opera_extensions";

	#######################################################################################
	#query["osquery::os_version"] =  "SELECT name,patch,build FROM os_version";

	#######################################################################################
	#query["osquery::passwd_changes"] =  "SELECT target_path,action FROM passwd_changes";

	#######################################################################################
	#query["osquery::pci_devices"] =  "SELECT pci_slot,driver,vendor,model FROM pci_devices";

	#######################################################################################
	#query["osquery::process_envs"] =  "SELECT pid,key,value FROM process_envs";

	#######################################################################################
	#query["osquery::process_memory_map"] =  "SELECT pid,permissions,device FROM process_memory_map";

	#######################################################################################
	#query["osquery::process_open_files"] =  "SELECT pid,fd,path FROM process_open_files";

	#######################################################################################
	query["osquery::process_open_sockets"] =  "SELECT pid,socket,protocol FROM process_open_sockets";

	#######################################################################################
	#query["osquery::processes"] =  "SELECT pid,name,path,cmdline FROM processes";

	#######################################################################################
	#query["osquery::routes"] =  "SELECT destination,source,interface FROM routes";

	#######################################################################################
	query["osquery::shell_history"] =  "SELECT username,command FROM shell_history";

	#######################################################################################
	#query["osquery::smbios_tables"] =  "SELECT number,description,size FROM smbios_tables";

	#######################################################################################
	query["osquery::system_controls"] =  "SELECT name,oid,subsystem FROM system_controls";

	#######################################################################################
	#query["osquery::uptime"] =  "SELECT days,hours FROM uptime";

	#######################################################################################
	query["osquery::usb_devices"] =  "SELECT usb_address,vendor,model FROM usb_devices";

	########################################################################################
	#query["osquery::user_groups"] =  "SELECT uid,gid FROM user_groups";

	########################################################################################
	#query["osquery::users"] =  "SELECT username,uid,gid FROM users";
...
event BrokerComm::incoming_connection_established(peer_name: string)
{
	...
	osquery::groupsubscribe("/bro/event/host2",query,"ADD");
